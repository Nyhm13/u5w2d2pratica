package it.epicode.u5w2d2pratica;
...
public class AppConfig {
     ...
     * le proprieta per accedere al servizio e poi questa mappa va  passata al costruttore di Cloudinary per instanziarlo*/
    public Cloudinary getCloudinary(@Value("${cloudinary.cloud_name}") String cloudName,
                                    @Value("${cloudinary.api_key}")String apiKey,
                                    @Value("${cloudinary.api_secret}")String apiSecret){
        // creiamo una mappa di stringhe per passarle le coppie di stringhe e valori  da ripassare poi in claudinary
